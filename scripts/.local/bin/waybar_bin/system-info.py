#!/home/amir/.local/bin/.venv/bin/python
import os
import json
import psutil
import GPUtil
...
def get_system_info_config(config_path: str):
    ...
    cpu_label_mode = config.get(
        'DEFAULT', 'cpu-label-mode', fallback='utilization')
    gpu_label_mode = config.get(
        'DEFAULT', 'gpu-label-mode', fallback='utilization')
...
if __name__ == "__main__":
    ...
    SESSION_TYPE = (lambda s: s if s != "$XDG_SESSION_TYPE" else None)(
        expandvars("$XDG_SESSION_TYPE"))
